...
function getPeople(courseId, personType){
  ...
  for (var i = 0; i < people[personType].length; i++){
    ...
    studentUserName = person['emailAddress'].replace("@"+DEFAULT_EMAIL_DOMAIN_NAME,"") //keeps domain if different from set domain
...
function emailStringToArray(string){
  ...
  for (let str of stringArray) {
    ...
    //information2 = information.match("(?<=<)(.*)(?=@)")[2]; //takes only the text inbetween < and @ with regex
    information2 = information.match("^([A-z][a-z]*).* ([A-z][a-z]*) <(.*)>$"); // gets info from regex, does't work for people with multiple first/last names
    informationArray.push([information2[3],...information2.slice(1,3)]); //adds array to main array
  }
...
function setRange(data,range){  
  ...
  console.log(range[2])
  console.log(`${range[2]+range[3]}:${range[4]}${parseInt(range[3])+data.length-1}`)
